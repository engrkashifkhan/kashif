...
const Hero = ({ fadeInUp, handleScroll }) => {
  return (
    <section
      ...
      className="relative min-h-screen flex items-center bg-white dark:bg-[#0A0A0A] overflow-hidden transition-colors duration-300"
    >
      {/* Subtle grid background */}
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.05)_1px,transparent_1px)] dark:bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:32px_32px] pointer-events-none"
      />

      {/* Ambient accent blurs */}
      <div
        aria-hidden="true"
        className="absolute top-20 right-10 w-64 h-64 bg-emerald-500/10 rounded-full blur-[80px] pointer-events-none"
      />

      <div
        aria-hidden="true"
        className="absolute bottom-20 left-10 w-72 h-72 bg-blue-600/10 rounded-full blur-[100px] pointer-events-none"
      />
      ...
      <div className="container mx-auto px-6 py-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <h1
              id="hero-heading"
              className="text-5xl md:text-6xl lg:text-7xl font-bold text-slate-900 dark:text-white tracking-tight mb-3"
            >
              Kashif{' '}
            ...
            <div
              aria-hidden="true"
              className="h-0.5 w-16 bg-gradient-to-r from-emerald-400 to-blue-500 mb-6"
            />
            ...
            <div className="text-slate-500 dark:text-slate-400 max-w-2xl text-base md:text-lg leading-relaxed mb-8 space-y-4">

              <p>
                I'm Kashif Khan, a Software Engineer and AI Engineer
                specializing in modern web development, MERN Stack
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {[
                ...
                <span
                  key={skill}
                  className="px-3 py-1.5 text-xs font-medium rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-white/5"
                >
                  {skill}
                </span>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <motion.button
                type="button"
                onClick={() => handleScroll('contact')}
                aria-label="Contact Kashif Khan"
                className="px-8 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-black font-semibold rounded-lg hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors shadow-lg shadow-black/5 dark:shadow-white/5"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
              ...
              <motion.button
                type="button"
                onClick={() => handleScroll('skills')}
                aria-label="View Kashif Khan's technical skills"
                className="px-8 py-3.5 text-slate-600 dark:text-slate-300 font-medium rounded-lg border border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
          ...
          >
            ...
            <div className="relative bg-slate-50 dark:bg-[#111] p-2 rounded-2xl border border-black/5 dark:border-white/10 shadow-2xl shadow-slate-200 dark:shadow-black/50">
              ...
              <img
                src="/kashif1.jpeg"
                alt="Kashif Khan, Software Engineer and AI Engineer"
                width="320"
                height="320"
                fetchPriority="high"
                decoding="async"
              ...
              <div
                aria-hidden="true"
                className="absolute -top-2 -left-2 w-6 h-6 border-t-2 border-l-2 border-emerald-400 rounded-tl-lg"
              />

              <div
                aria-hidden="true"
                className="absolute -bottom-2 -right-2 w-6 h-6 border-b-2 border-r-2 border-blue-500 rounded-br-lg"
              />
